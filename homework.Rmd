---
title: "Untitled"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

```{r}
x=read.csv('./data/DailyDelhiClimateTrain.csv')
tem=ts(x$meantemp)
```

```{r}
plot(tem)
```

```{r}
library(xts)
tem=xts(x$meantemp,as.Date(x$date))
plot(tem)
```

```{r}
acf(tem)
```







```{r}
library(fUnitRoots)
library(forecast)
```


```{r}
diff_tem=diff(tem)[-1]
plot(diff_tem)
adfTest(diff_tem)
acf(diff_tem)
pacf(diff_tem)
```

```{r}
x=diff(diff_tem,365)[-1:-365]
head(x)
plot(x)
adfTest(x)
acf(x)
pacf(x)
```

```{r}
ans=auto.arima(x)
auto.arima(x)
```
$x_t=ma1*a_{t-1}+ma2*a_{t-2}$
```{r}
plot(ans$fitted)
```
```{r}
pred_x=forecast(ans,365)
autoplot(forecast(ans,365))
```

```{r}
e=ans$x-ans$fitted
plot(e)
library(FinTS)
library(fGarch)
ArchTest(e,lags = 5)
```
```{r}
ans_garch=garchFit(~arma(1, 2)+garch(5,0),data=x)
summary(ans_garch)
```

```{r}
ans_garch=garchFit(~arma(1, 1)+garch(1,0),data=x)
summary(ans_garch)
```
```{r}
e=residuals(ans_garch)
plot(density(e))

```
```{r}
Box.test(e^2,lag = 1,type = "Ljung-Box")
acf(e^2)
pacf(e^2)
```

```{r}
pred_x=predict(ans_garch,114)
```


```{r}
library(zoo)
library(xts)

pred_tem=as.numeric(tem)
n=length(tem)+1
for (i in n:(n+113)) {
  pred_tem=c(pred_tem,pred_x$meanForecast[i-n+1]
             +pred_tem[i-1]+pred_tem[i-365]
             -pred_tem[i-366])
}
pred_tem_up=as.numeric(tem)

for (i in n:(n+113)) {
  pred_tem_up=c(pred_tem_up,pred_x$meanForecast[i-n+1]+3*pred_x$standardDeviation[i-n+1]
                +pred_tem[i-1]+pred_tem[i-365]
                -pred_tem[i-366])
}
pred_tem_low=as.numeric(tem)

for (i in n:(n+113)) {
  pred_tem_low=c(pred_tem_low,pred_x$meanForecast[i-n+1]-3*pred_x$standardDeviation[i-n+1]
                 +pred_tem[i-1]+pred_tem[i-365]
                 -pred_tem[i-366])
}

dates=seq(as.Date("2013-01-01"), length = length(pred_tem), by = "days")

pred_tem=xts(pred_tem,dates)[n:(n+113)]
pred_tem_up=xts(pred_tem_up,dates)[n:(n+113)]
pred_tem_low=xts(pred_tem_low,dates)[n:(n+113)]
plot(pred_tem)
```




```{r}
test_data=read.csv("./data/DailyDelhiClimateTest.csv")
real_tem=xts(test_data$meantemp,as.Date(test_data$date))
plot(real_tem)
```
```{r}
xx=merge(pred_tem,pred_tem_low,pred_tem_up,real_tem)
pred=xx
plot(pred,col=c("blue","grey","grey","red"))
```
```{r}
library(data.table)
library(ggplot2)
xxx=data.table(xx)
xxxx=cbind(x=1:114,xxx)
df=melt(xxxx,id="x")
p=ggplot(df,aes(x=x,y=value,group=variable,color=variable))+
  geom_line()
p

```

