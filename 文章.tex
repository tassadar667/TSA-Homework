...
\begin{titlepage}
    ...
    \begin{center}
        ...
        \begin{minipage}[h]{.8\textwidth}
            \centering{\heiti\zihao{2}对于德里每日平均气温的时间序列分析}
        \end{minipage}
        \vskip 20bp
        \vspace*{\stretch{3}}
        ~~\\
        ...
        \zihao{-3}\heiti
        \begin{tabular}{cc}
            学~~~院~~~名~~~称~~ & \underline{~~~~经济管理学院~~~~} \\[.4ex]
            专~~~业~~~名~~~称~~ & \underline{~~~~~~~统计~~~~~~~~} \\[.4ex]
            学~~~生~~~姓~~~名~~ & \underline{~~~~~~李俊然~~~~~~~} \\[.4ex]
            ~~~~~学~~~号~~~~~~~ & \underline{~~~~~18377198~~~~~}  \\
        \end{tabular}

        }
        \vskip 60bp
        \centerline{\heiti\zihao{-3}2021~~年~~6~~月~~12~~日}
    \end{center}
\end{titlepage}
...
\begin{figure}[H]
    \centering % 图片居中
    \includegraphics[width = 8cm]{figure/02.eps}
    \caption{X时间序列图}
    \label{x_seq}
\end{figure}
...
对于残差，考虑是否存在条件异方差，对于残差序列进行了Ljung-Box检验，由于在PACF图中为5步截尾，
Ljung-Box检验步幅设置为5，在显著性水平为0.05的条件下拒接了原假设，认为序列存在Arch效应，
因此接下来对模型进行改进，建立ARAM+Arch模型


\subsection{ARAM+Arch模型}
首先，我们建立了arma(1, 2)+arch(5)模型，使用R中fGarch包进行了模型参数求解，
得到的结果中部分参数不够显著。


去掉模型中不显著的参数（显著性水平为0.05），模型简化为arma(1, 1)+arch(1)，重新进行参数求解，
得到的结果如下
...
\section{模型预测检验}
测试集给出了2017年1月1日到4月24日的数据，利用过去的数据进行预测，上下界为$\mu\pm3\sigma$

\begin{figure}[H]
    \centering % 图片居中
    \includegraphics[width = 8cm]{figure/10.eps}
    \caption{预测效果}
\end{figure}
...
\appendix
\section{附录}
本文数据以及代码在https://github.com/tassadar667/TSA-Homework。
